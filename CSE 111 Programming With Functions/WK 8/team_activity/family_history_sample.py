...
def main():
    ...
    # Print a blank line.
    print(NAME_INDEX, DEATH_YEAR_INDEX)
...
def count_genders(people_dict):
 ...
 # Initialize counter variables
    ...
    for person_data in people_dict.values():
        ...
        if person_data[GENDER_INDEX] == "M":
            male_count += 1
        
        else:
            female_count += 1

    print("Total number by gender")
    print(f"Males are: {male_count}")
    print(f"Females are: {female_count}")
...
def print_marriages(marriages_dict, people_dict):
    ...
    for person_key, mariage_data in marriages_dict.items():

        # Get the husband person key, wife person
        # key, and year they were married.
        husband_key = mariage_data[HUSBAND_KEY_INDEX]
        wife_key = mariage_data[WIFE_KEY_INDEX]
        wedding_year = mariage_data[WEDDING_YEAR_INDEX]
